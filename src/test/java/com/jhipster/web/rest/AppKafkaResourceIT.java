...
import com.jhipster.config.KafkaSseConsumer;
import com.jhipster.config.KafkaSseProducer;
...
class AppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/app-kafka/publish?message=value-produce")).andExpect(status().isOk());
